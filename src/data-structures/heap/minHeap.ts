...
/**
 * 최소 힙 구조를 배열로 구현한 클래스입니다.
 *
 * 힙 클래스를 바탕으로 작성되었습니다.
 *
 * 따라서, 부모 노드의 값 혹은 위계가 자식 노드의 값 혹은 위계보다 작은 구조입니다.
 *
 * @class MinHeap
 * @extends Heap
...
export class MinHeap<T = any> {
  ...
  constructor(getCmpValue: Comparator<T>, _heap?: Heap<T>) {
    ...
    this._heap = _heap || new Heap<T>(createMinCmp<Comparator<T>>(getCmpValue));
  }
   ...
   * ### clone 메서드
   *
   * 해당 최소 힙의 얕은 복사된, 새로운 최소 힙 인스턴스를 반환합니다.
   */
  clone() {
    return new MinHeap<T>(this._getCmpValue, this._heap);
  }
  ...
  public static heapify<T = any>(cmp: Comparator<T>, values: T[]) {
    ...
    const __heap = new Heap<T>(createMinCmp<Comparator<T>>(cmp), values);

    return new MinHeap<T>(cmp, __heap).heapify();
  }

  public static isHeap<T = any>(cmp: Comparator<T>, values: T[]) {
    const __heap = new Heap<T>(createMinCmp<Comparator<T>>(cmp), values);
    return new MinHeap<T>(cmp, __heap).isHeap();
  }
